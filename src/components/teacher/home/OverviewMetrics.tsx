...
export interface MetricStats {
    totalOnlineClass?: number | string;
    totalOnsiteClass?: number | string;
    totalStudents?: number | string;
    totalGraduated?: number | string;
}
...
function makeMetric(id: string, number: string | number, title: string, icon: React.ReactNode): MetricItem {
    return { id, number, title, icon };
}

export default function OverviewMetrics({ stats, metrics }: OverviewMetricsProps) {
    const defaultMetrics: MetricItem[] = [
        makeMetric(
            'online-class',
            stats?.totalOnlineClass ?? 0,
            'Total Online class',
    ...
    const computedMetrics: MetricItem[] = metrics || defaultMetrics;
