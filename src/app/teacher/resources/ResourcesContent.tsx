...
export interface ResourcesContentProps {
    initialResources?: ResourceItem[];
    ShowTab?: boolean;
    showTab?: boolean;
    styleSearch?: 'style1' | 'style2';
    'style-search'?: 'style1' | 'style2';
    tag?: string;
    title?: string;
    /** Số lượng hiển thị ban đầu (mặc định: 8) */
    limit?: number;
    count?: number;
    displayCount?: number;
    initialCount?: number;
...
export default function ResourcesContent({
    ...
    pageSize,
    soLuongHienThi,
    'display-count': displayCountKebab,
}: ResourcesContentProps = {}) {
    const allResources = initialResources || [];
    const isShowTab = ShowTab !== undefined ? ShowTab : showTab !== undefined ? showTab : true;
    const activeStyleSearch = styleSearchKebab || styleSearch;
    const searchBoxClass = `${styles['resources-content__search-box']} ${
        activeStyleSearch === 'style2' ? styles['resources-content__search-box--style2'] : ''
    }`;
    ...
    const [visibleCount, setVisibleCount] = useState<number>(effectiveLimit);

    useEffect(() => {
        setVisibleCount(effectiveLimit);
    }, [effectiveLimit]);
    ...
    const filteredResources = useMemo(() => {
        return allResources.filter((res) => {
            const matchesTab = !isShowTab || activeTab === 'all' || res.category === activeTab;
            const matchesSearch =
                searchQuery.trim() === '' ||
    ...
    }, [allResources, activeTab, searchQuery, isShowTab]);

    const displayedResources = filteredResources.slice(0, visibleCount);

    const handleLoadMore = () => {
        const step = effectiveLimit > 0 ? effectiveLimit : 4;
        setVisibleCount((prev) => Math.min(prev + step, filteredResources.length));
    };

    return (
        <section className={styles['resources-content']}>
            <div className={styles['resources-content__container']}>
                {/* 1. Header with Reusable DashboardHeadings & Search Box when tabs are hidden */}
                <div
                    className={`${styles['resources-content__header-row']} ${
                        !isShowTab ? styles['resources-content__header-row--no-tabs'] : ''
                    }`}
                >
                    <DashboardHeadings
                        tag={tag}
                        title={title}
                        align="left"
                        className={styles['resources-content__heading']}
                    />
                ...
                {/* 2. Navigation Row: Filter Tabs & Search Box (shown when ShowTab is true) */}
                {isShowTab && (
